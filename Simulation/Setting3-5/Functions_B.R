...
# version 2
get_best_partition_2 <- function(dataset,
                                 model_formula,
                                 partition_var,
                                 p.value = 0.05,
                                 level_conti = 20,
                                 min_N=30){
  # usage: get_best_partition(wideData,e~factor(Z)*factor(trt)*time+us(time_fac|id),c("X1","X2","X3","X4","X5"))
  parti_list <- get_all_partys(dataset,partition_var,level_conti = level_conti,min_N = min_N)
  outcome_list <- list()
  p_list <- list()
  if (length(parti_list) > 0){
    for (i in 1:length(parti_list)) {
      ...
      longData_cur <- longData_cur[order(longData_cur$trt,longData_cur$id,longData_cur$time),]
      K <- length(grep(pattern = "e",names(dataset),value = TRUE))
      longData_cur$time_fac <- factor(longData_cur$time)
      longData_cur$id <- factor(longData_cur$id)
      mmrm_cur <- mmrm(model_formula,data = longData_cur)
      b <- length(mmrm_cur$beta_est)
      a <- b - 3
      est <- mmrm_cur$beta_est
      vcova <- mmrm_cur$beta_vcov
      outcome_cur <- (est[a] + K * est[b])^2 / (vcova[a,a] + (K^2) * vcova[b,b] + 2*K*vcova[a,b])
      p_cur <- 1-pchisq(outcome_cur,1)
      outcome_list[i] <- outcome_cur
      p_list[i] <- p_cur
    }
...
MMRM_tree <- function(dataset,
  ...
  if (Prunemethod == "None" | Prunemethod == "Bootstrap" | Prunemethod == "Regular") {
    ...
    if (Prunemethod == "None"){
      t2 <- Sys.time()
      #print(paste("computation time:",t2-t1))
      return(MMRM_try)
    }
  ...
  if (Prunemethod == "Testset"){
    #train_id <- 1:round(2 / 3 * nrow(dataset))
    #test_id <- (round(2 / 3 * nrow(dataset))+1):nrow(dataset)
    train_id <- (round(1 / 3 * nrow(dataset))+1):nrow(dataset)
    test_id <- 1:round(1 / 3 * nrow(dataset))
    trainset <- dataset[train_id, ]
    testset <- dataset[test_id, ]
...
find_ri_lmer <- function(data.lmetree,N){
  a <- data.lmetree$data
  ri <- c()
  for (i in 1:N) {
