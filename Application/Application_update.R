...
t1 <- Sys.time()
set.seed(9)
tr <- MMRM_tree(wideData_2,e~BL + factor(Z)*factor(trt)*factor(time)+us(time|id),
...
tr <- MMRM_tree(wideData_2,e~BL + factor(Z)*factor(trt)*time+us(time_fac|id),
                c("BL","AGE","GENDER","MMSCORE","COTOTSCR","ADTOTSCR","EVGDS","GENOTYPE"),
                Prunemethod = "None",level_conti = 30,min_N = 30,p.value = 0.1)
...
subgroup_2 <- wideData_2[wideData_2$MMSCORE > 24 & wideData_2$BL <= 1.5 & wideData_2$COTOTSCR <= 13,]
dim(subgroup_2)
sum(subgroup_2$trt == 1)
sum(subgroup_2$trt == 0)
subgroup_3 <- wideData_2[wideData_2$MMSCORE > 24 & wideData_2$BL <= 1.5 & wideData_2$COTOTSCR > 13,]
dim(subgroup_3)
sum(subgroup_3$trt == 1)
sum(subgroup_3$trt == 0)
subgroup_4 <- wideData_2[wideData_2$MMSCORE > 24 & wideData_2$BL > 1.5 & wideData_2$AGE <= 75,]
dim(subgroup_4)
sum(subgroup_4$trt == 1)
sum(subgroup_4$trt == 0)
subgroup_5 <- wideData_2[wideData_2$MMSCORE > 24 & wideData_2$BL > 1.5 & wideData_2$AGE > 75,]
sum(subgroup_5$trt == 1)
sum(subgroup_5$trt == 0)
subgroup_combine <- rbind(subgroup_1,subgroup_4)
...
mmrm_cur <- mmrm(e~BL + factor(trt)*factor(time)+us(time|id),data = longData_cur)
contrast <- numeric(length(mmrm_cur$beta_est))
contrast[3] <- contrast[13] <- 1
