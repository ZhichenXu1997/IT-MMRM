...
get_best_partition_IT <- function(dataset,
                               model_formula,
                               partition_var,
                               p.value = 0.05,
                               level_conti = 20,
                               min_N=30){
  # usage: get_best_partition_IT(wideData,e4~factor(Z)*factor(trt),c("X1","X2","X3","X4","X5"))
  ...
  if (length(parti_list) > 0){
    for (i in 1:length(parti_list)) {
      data_cur <- dataset
      data_cur$Z <- kidids_split(parti_list[[i]], data = data_cur)-1
      model_cur <- lm(model_formula,data = data_cur)
      l <- length(model_cur$coefficients)
      outcome_cur <- (model_cur$coefficients[l])^2 / vcov(model_cur)[l,l]
...
IT_grow <- function(dataset,
                      ...
                      level_conti = 20,
                      min_N = 30){
  # usage: IT_try <- IT_grow(wideData,e4~factor(Z)*factor(trt),c("X1","X2","X3","X4","X5"))
  if (depth > max_depth || length(unique(dataset[, "id"])) < min_N) {
    return(partynode(id = strtoi(gsub("2", "0", name), base = 2)))
  }
  split <- get_best_partition_IT(dataset,
                              model_formula,
                              partition_var,
                              p.value = p.value,
  ...
  if (length(split) == 0) {
    ...
    kids[[1]] <- IT_grow(dataset = dataset[kidids == 1,],
                           model_formula,
                           partition_var,
                           depth = depth + 1,
                           max_depth = max_depth,
    ...
    kids[[2]] <- IT_grow(dataset = dataset[kidids == 2,],
                           model_formula,
                           partition_var,
                           depth = depth + 1,
                           max_depth = max_depth,
...
GetGt <- function(nd) {
  # usage: GetGt(IT_try)   GetGt(IT_try$kids[[1]])
...
OptimalSelf <- function(prune.seq, lambda) {
  # usage: OptimalSelf(Prune(IT_try),4)[[1]]$node
...
GetGtNew_IT <- function(nd,
                     dataset,
                     model_formula) {
  # usage: GetGtNew_IT(IT_try,test_set,e4~factor(Z)*factor(trt))
  if (is.null(nd) || is.null(nd$split)) {
    return(0)
  }
  grp <- kidids_split(nd$split, data = dataset)-1
  data_cur <- dataset
  data_cur$Z <- kidids_split(nd$split, data = data_cur)-1
  model_cur <- lm(model_formula,data = data_cur)
  l <- length(model_cur$coefficients)
  outcome_cur <- (model_cur$coefficients[l])^2 / vcov(model_cur)[l,l]
  p_cur <- 1-pchisq(outcome_cur,1)
...
AlphaCalculateNew_IT <- function(nd,dataset,model_formula) {
  # usage: AlphaCalculateNew(IT_try,test_set,e~factor(Z)*factor(trt)*factor(time)+us(time|id))
  if (is.null(nd) || is.null(nd$split)) {
    return(list())
  }
  left <- AlphaCalculateNew_IT(nd$kids[[1]],dataset=dataset,model_formula=model_formula)
  right <- AlphaCalculateNew_IT(nd$kids[[2]],dataset=dataset,model_formula=model_formula)
  record <- list(list(node = nd, alpha = GetGtNew_IT(nd=nd,dataset=dataset,model_formula=model_formula) / NodeCount(nd)))
  if (length(left) > 0) {
    record <- c(record, left)
  }
  if (length(right) > 0) {
...
PruneNew_IT <- function(nd,dataset,model_formula) {
  # create a sequence of pruned nodes
  # usage: PruneNew_IT(IT_try,test_set,e~factor(Z)*factor(trt)*factor(time)+us(time|id))
  record <- list(list(node = nd, alpha = GetGtNew_IT(nd=nd,dataset=dataset,model_formula=model_formula) / NodeCount(nd)))
  
  while (depth(nd) > 0) {
    seq <- AlphaCalculateNew_IT(nd=nd,dataset=dataset,model_formula=model_formula)
    weak <-
      seq[[which.min(lapply(seq, function(x) {
        return(x$alpha)
      }))]]
...
OptimalSelfNew_IT <- function(prune.seq,dataset,model_formula,lambda) {
  # usage: OptimalSelfNew(PruneNew(IT_try,test_set,e~factor(Z)*factor(trt)*factor(time)+us(time|id)),test_set,e~y0 + factor(Z)*factor(trt)*factor(time)+us(time|id),4)[[1]]$node
  targets <- lapply(prune.seq, function(x) {
    return(GetGtNew_IT(x$node,dataset = dataset,model_formula = model_formula) - lambda * NodeCount((x$node)))
  })
  prune.seq[which.max(targets)]
}


BootstrapPrune_IT <- function(dataset,
                           tr,
                           model_formula,
                           partition_var,
                           max_depth = 3,
                           ...
                           B = 20){
  # usage: BootstrapPrune_IT(wideData,IT_try,e~factor(Z)*factor(trt),c("X1","X2","X3","X4","X5"))
  prune.seq <- Prune(tr)
...
IT_tree <- function(dataset,
                      ...
                      min_N = 30,
                      B = 20){
  # usage: IT_tree(wideData,e4~factor(Z)*factor(trt),c("X1","X2","X3","X4","X5"),Prunemethod = "None",lambda = 4)
  if (Prunemethod == "None" | Prunemethod == "Bootstrap" | Prunemethod == "Regular") {
    IT_try <- IT_grow(dataset = dataset,
                          model_formula = model_formula,
                          partition_var = partition_var,
                          max_depth = max_depth,
                          p.value = p.value,
    ...
    if (Prunemethod == "Bootstrap"){
      tr <- BootstrapPrune_IT(dataset = dataset,
                           IT_try,
                           model_formula = model_formula,
                           partition_var = partition_var,
                           max_depth = max_depth,
    ...
    if (Prunemethod == "Regular"){
      tr <- OptimalSelf(Prune(IT_try),lambda)[[1]]$node
      return(tr)
    }
  }
  if (Prunemethod == "Testset"){
    ...
    IT_try <- IT_grow(dataset = trainset,
                          model_formula = model_formula,
                          partition_var = partition_var,
                          max_depth = max_depth,
                          p.value = p.value,
    ...
    #print("finish")
    #print(IT_try)
    tr <- OptimalSelfNew_IT(PruneNew_IT(IT_try,testset,model_formula),testset,model_formula,lambda)[[1]]$node
    return(tr)
  }
}